#include "aes.h"

static word rrot32(word w, unsigned int k)
{
  return (w << (32-k)) | (w >> k);
}

static void mem_copy(void *restrict dst, const void *restrict src, int len)
{
  char *d;
  const char *s;
  int i;
  d = dst;
  s = src;
  for (i = 0; i < len; i += 1) {
    d[i] = s[i];
  }
}

static byte S[] = {
  
};

static word sub_word(word w)
{
  int i;
  byte *b;
  b = (byte *)&w;
  for (i = 0; i < 4; i += 1) {
    b[i] = S[b[i]];
  }
  return w;
}

static word rot_word(word w)
{
  return rrot32(w, 8);
}

static byte Rcon[] = {
  0x00,                         /* unused */
  0x01, 0x02, 0x04, 0x08, 0x10, 0x20, 0x40, 0x80, 0x1b, 0x36,
  0x6c, 0xd8, 0xab, 0x4d        /* only for larger keys */
};

void aes_init(aes_ctx *ctx, byte key[4*Nk])
{
  int i;
  word *w;
  w = ctx->w;
  mem_copy(w, key, 4 * Nk);
  for (i = Nk; i < (Nb * (Nr+1)); i += 1) {
    word tmp = w[i-1];
    if ((i % Nk) == 0) {
      tmp = sub_word(rot_word(tmp)) ^ Rcon[i/Nk];
    }
    w[i] = w[i-Nk] ^ tmp;
  }
}

static void sub_bytes(byte state[4*Nb])
{
  int i;
  for (i = 0; i < 4*Nb; i += 1) {
    state[i] = S[state[i]];
  }
}

static word get_row(byte state[4*Nb], int i)
{
  return ((state[i]) |
          (state[i+4] << 8) |
          (state[i+8] << 16) |
          (state[i+12] << 24));
}

static void set_row(byte state[4*Nb], int i, word w)
{
  int j;
  state[i] = w & 0xff;
  for (j = 1; j < Nb; j += 1) {
    state[i+(j*4)] = (w & (0xff << (j*8))) >> (j*8);
  }
}

static void shift_rows(byte state[4*Nb])
{
  int i;
  word w;
  for (i = 1; i < Nb; i += 1) {
    w = get_row(state, i);
    set_row(state, i, rrot32(w, i * 8));
  }
}

static byte M2[] = {
  
};

static byte M3[] = {
  
};

static void mix_columns(byte state[4*Nb])
{
  int i;
  word tmp;
  byte *b;
  for (i = 0; i < Nb; i += 1) {
    tmp = *(word *)(state + (i*4));
    b = (byte *)&tmp;
    state[0+(i*4)] = M2[b[0]] ^ M3[b[1]] ^ b[2] ^ b[3];
    state[1+(i*4)] = b[0] ^ M2[b[1]] ^ M3[b[2]] ^ b[3];
    state[2+(i*4)] = b[0] ^ b[1] ^ M2[b[2]] ^ M3[b[3]];
    state[3+(i*4)] = M3[b[0]] ^ b[1] ^ b[2] ^ M2[b[3]];
  }
}

static void add_round_key(byte state[4*Nb], word w[Nb])
{
  int i;
  word *wp;

  for (i = 0, wp = (word *)state; i < Nb; i += 1) {
    wp[i] ^= w[i];
  }
}

void aes_encrypt(aes_ctx *ctx, byte in[4*Nb], byte out[4*Nb])
{
  int i;
  byte *state;
  word *wp;

  state = out;
  mem_copy(state, in, 4*Nb);

  wp = ctx->w;
  add_round_key(state, wp);

  for (i = 1; i < Nr; i += 1) {
    sub_bytes(state);
    shift_rows(state);
    mix_columns(state);
    add_round_key(state, wp + (i*4));
  }

  sub_bytes(state);
  shift_rows(state);
  add_round_key(state, wp + (i*4));
}
